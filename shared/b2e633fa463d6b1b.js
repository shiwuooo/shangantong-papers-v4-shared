...
window.UBB = (function () {
        ...
        Util = {
            ...
            parseNode: function(node, nodeName, nodeType, setting, re, state) {
                ...
                for (tagName in setting.tags) {
                    if (setting.tags.hasOwnProperty(tagName)) {
            ...
            scanUbbText: function(text, setting, needEncodeHtml) {
                ...
                text = text.replace(/\r\n/g, '\n'); // for IE hack
                var c, r, tagName, tag, attr, isClose,
                ...
                if (node.name === '#text') {
                    if (node.value === '\n') {
                        if (state.nobr) {
                            state.nobr = false;
                            return '';
                        } else if (!node.parent || node.parent.name !== 'tex') {
                            return '<br/>';
                        } else {
                            return node.value;
                        }
                        ...
                        state.nobr = false;
                        if (!node.parent || node.parent.name !== 'tex') {
             ...
             * @return {string} ubb text of node and it's children
             */
            fixUbbNode: function(node, sonString) {
                if (node.name === '#text') {
        ...
        parseHtml = function(node, setting, state, notRoot) {
            ...
            if (nodeType === 1) {
                ...
                state.textStates.push({
                    ...
                    keepWhiteSpace: Util.isKeepWhiteSpace(node)
                });
            }

            // parse children
            if (!($(node).hasClass('um-tab') || $(node).attr('lax-content'))) {
    ...
    function UBB(setting) {
        this.setting = UBB.mix({
            ...
            formulaPrefix: '/accessories/formulas?latex=',
            imagePrefix: '/images/'
        }, setting);
        ...
        for (k in setting.tags) {
            if (setting.tags.hasOwnProperty(k)) {
                v = setting.tags[k];
    ...
    UBB.prototype.parseHtml = function (html) {
        return parseHtml(html);
    }

    UBB.prototype.fixUbb = fixUbb;
    return UBB;
})();
